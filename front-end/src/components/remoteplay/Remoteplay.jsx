...
//       this.sendKeys(keys);
//       const websocketAddress = "ws://192.168.100.48:8080"; // NestJS WebSocket 서버 주소
//       const socket = new WebSocket(websocketAddress);
      
//       socket.onopen = () => {
//         console.log("Good");
//         const message = {
//           message:'HI, I am React',
//         };
        
//         socket.send('enableKeyCapture',message);
//         console.log("finish");
//       }; 
//     }); 
//     if (this.datachannel) {   
...
import React, { Component } from 'react';
import { Button, Tooltip, OverlayTrigger, Accordion, Form } from 'react-bootstrap';
import WebrtcSession from './webrtc';
import Keyboard from './keyboard';
import { io } from 'socket.io-client';
...
class SessionManager extends Component {
  ...
  async handleClick() {
    if (this.state.isToggleOn) {
      try {
        this.session = new WebrtcSession('ws://192.168.100.250:8888/webrtc', this.props.options);
        this.session.setOnStreamCallback(this.props.onStream);
        this.session.setOnCloseCallback(() => {
  ...
  render() {
    const tooltip = (
      <Tooltip id="tooltip">
        Start the screen sharing session. Keyboard events will then be sent to the remote peer.
      </Tooltip>
    );
...
class ScreenSharing extends Component {
  constructor(props) {
    super(props);
    this.handleOptionsChange = this.handleOptionsChange.bind(this);
    this.handleKeyCaptureChange = this.handleKeyCaptureChange.bind(this);
    this.onStream = this.onStream.bind(this);
    this.videoRef = React.createRef();
    this.sessionRef = React.createRef();
    this.state = {
  ...
  handleKeyCaptureChange(keyCapture) {
    this.setState({ keyCapture });
  }
      ...
      <div>
        ...
        <div>
          <Options
            ...
            onOptionsChange={this.handleOptionsChange}
            keyCapture={this.state.keyCapture}
            onKeyCaptureChange={this.handleKeyCaptureChange}
          />
        </div>
        <SessionManager
          ref={this.sessionRef}
